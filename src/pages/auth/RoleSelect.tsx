  ...
  return (
    <AuthBackground>
      ...
      <div className="flex-1 min-h-0 flex flex-col">
        <div className="flex-1 min-h-0 overflow-y-auto no-scrollbar px-6 pb-2">
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            className="mb-4 sm:mb-5"
          >
            <h1 className="text-2xl sm:text-3xl font-display font-extrabold text-white mb-2 sm:mb-3 leading-tight">
          ...
          <div className="relative min-h-[210px] h-[min(38dvh,300px)] max-h-[300px] mb-3 sm:mb-4">
        ...
        <div className="flex-shrink-0 px-6 pt-3 pb-[max(1.25rem,env(safe-area-inset-bottom))] bg-gradient-to-t from-black/55 via-black/35 to-transparent space-y-3">
          ...
          <button
            ...
            className="w-full py-3 border-2 border-dashed border-white/40 rounded-2xl text-white font-bold text-sm hover:bg-white/10 transition-colors flex items-center justify-center gap-2"
          >
            <svg
...
function RoleCarouselCard({ role }: { role: (typeof ROLES)[number] }) {
  return (
    <Card
      leaves
      leafOpacity={95}
      className="h-full p-4 sm:p-6 flex flex-col bg-forest border-2 border-white/25 shadow-float"
    >
      <div className="relative z-0 flex items-start gap-3 sm:gap-5">
        <div className="w-16 h-16 sm:w-[4.5rem] sm:h-[4.5rem] rounded-3xl bg-white/15 border border-white/20 flex items-center justify-center text-3xl sm:text-4xl shadow-sm flex-shrink-0">
          {role.icon}
        </div>
        <div className="flex-1 min-w-0 pt-0.5 sm:pt-1">
          <h3 className="font-display font-bold text-lg sm:text-xl text-white mb-1 leading-tight">
            {role.title}
          </h3>
          <p className="text-xs sm:text-sm text-white/75 leading-relaxed">{role.desc}</p>
        </div>
      </div>

      <ul className="relative z-0 mt-3 sm:mt-5 pt-3 sm:pt-5 border-t border-white/20 space-y-2 flex-1 overflow-hidden">
        {role.highlights.map((item) => (
          <li key={item} className="flex items-start gap-2 text-xs sm:text-sm text-white/90">
            <span className="mt-1.5 w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-orange flex-shrink-0" />
